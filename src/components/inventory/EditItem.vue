  ...
  <div v-if="item" class="edit-item container">
    <v-form @submit.prevent="editItem">
      <v-card color="grey lighten-4">
        <v-toolbar dark height="100px" elevation="0" color="blue-grey
">
          <v-card-title>
        ...
        <v-card-text>
          <v-text-field label="Name" type="text" name="name" v-model="item.name" />
        </v-card-text>
        <v-card-text>
          <v-text-field label="Quantity" type="text" name="quantity" v-model="item.quantity" />
        </v-card-text>
         <v-card-text>
          <v-text-field label="Price per unit" type="text" name="ppu" v-model="item.ppu" />
        </v-card-text>
         <!-- <v-card-text>
          <v-text-field label="GST" type="text" name="gst" v-model="item.gst" />
        </v-card-text> -->
